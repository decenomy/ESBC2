...
void PrepareShutdown()
{
    ...
    RenameThread("esbc-shutoff");
    mempool.AddTransactionsUpdated(1);
    StopRPCThreads();
#ifdef ENABLE_WALLET
    if (pwalletMain)
        bitdb.Flush(false);
    GenerateESBCs(false, NULL, 0);
#endif
    StopNode();
    DumpMasternodes();
...
//    strUsage += HelpMessageOpt("-gmnotify=<cmd>", _("Execute command when a gm message is received (%s in cmd is replaced by message)"));
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "esbcoin.conf"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in esbcoin/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in esbcoin/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, gm ,bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, proxy, esbcoin, (obfuscation, swiftx, masternode, mnpayments)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in esbcoin/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all esbcoin specific functionality (Masternodes, Obfuscation, SwiftX) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-anonymizeesbcoinamount=<n>", strprintf(_("Keep N esbcoin anonymized (default: %u)"), 0));
...
/** Sanity checks
 *  Ensure that esbcoin is running in a usable environment with all
...
#endif
    // Make sure only a single esbcoin process is using the data directory.
...
// ********************************************************* Step 8: load wallet
    ...
    nAnonymizePhcAmount = GetArg("-anonymizeesbcoinamount", 0);
    ...
    LogPrintf("Anonymize esbcoin Amount %d\n", nAnonymizePhcAmount);
    ...
    /* Denominations
       ...
       For example:
       1esbcoin+1000 == (.1esbcoin+100)*10
       10esbcoin+10000 == (1esbcoin+1000)*10
    */
    ...
    */
    ...
    if (pwalletMain)
        GenerateESBCs(GetBoolArg("-gen", false), pwalletMain, GetArg("-genproclimit", 1));
#endif
