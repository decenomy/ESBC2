...
CMasternode* CMasternodeMan::GetNextMasternodeInQueueForPayment(int nBlockHeight, unsigned mnlevel, bool fFilterSigTime, unsigned& nCount)
{
    ...
    */
    ...
    for(CMasternode& mn : vMasternodes) {
        ...
        if (masternodePayments.IsScheduled(mn, nBlockHeight) && nMnCount >= 6)
            continue;
    ...
    int nTenthNetwork = (nMnCount >= 6) ? nMnCount / 10 : nMnCount;
    int nCountTenth = 0;
    uint256 nHigh = 0;
    BOOST_FOREACH (PAIRTYPE(int64_t, CTxIn) & s, vecMasternodeLastPaid) {
        CMasternode* pmn = Find(s.second);
        if (!pmn) { 
            LogPrint("masternode", "CMasternodeMan::GetNextMasternodeInQueueForPayment - pmn not found\n");
            break;
        } else {
            LogPrint("masternode", "CMasternodeMan::GetNextMasternodeInQueueForPayment - pmn found\n");
        }

        uint256 n = pmn->CalculateScore(1, nBlockHeight - 100);
        LogPrint("masternode", "CMasternodeMan::GetNextMasternodeInQueueForPayment - CalculateScore = %s\n", n.ToString());
        if (n > nHigh) {
            nHigh = n;
            pBestMasternode = pmn;
            LogPrint("masternode", "CMasternodeMan::GetNextMasternodeInQueueForPayment - selecting pBestMasternode\n");
        }
        nCountTenth++;
        LogPrint("masternode", "CMasternodeMan::GetNextMasternodeInQueueForPayment - nCountTenth: %d >= nTenthNetwork: %d\n", nCountTenth, nTenthNetwork);
